/*
MS5607.h
Library for barometric pressure sensor MS5607 on I2C with arduino

Original author for MS5607: Petr Gronat@2014
*/
#define OSR           2   // 0-3
...
void MS5607::getCalibration(uint16_t *C){
  for(uint8_t k=0;k<N_PROM_PARAMS;k++)
    C[k]=_C[k];
  return;
}

void MS5607::sendCommand(uint8_t cmd){
  Wire.beginTransmission(ADD_MS5607);
  Wire.write(cmd);
  Wire.endTransmission();
}

uint32_t MS5607::readnBytes(uint8_t nBytes){
  if (0<nBytes & nBytes<5){ 
    Wire.beginTransmission(ADD_MS5607);
    Wire.requestFrom((uint8_t)ADD_MS5607, nBytes);
      uint32_t data = 0;
